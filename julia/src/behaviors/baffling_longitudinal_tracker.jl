...
function set_desired_speed!(model::BafflingLongitudinalTracker, v_des::Float64)
    model.v_des = v_des
    model
end
function track_longitudinal!(model::BafflingLongitudinalTracker, v_ego::Float64, v_oth::Float64, headway::Float64)
    if !isnan(v_oth)
        ...
        Δv = (model.v_des + Base.rand(Float64(model.v_offset):Float64(model.v_offset)))- v_ego
